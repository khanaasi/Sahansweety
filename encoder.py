...
# --- ROBUST DOWNLOADER WITH AUTO-RETRY ---
async def download_tg_link(app_instance, link, output_path, step_name):
    try:
        # Link ko safely parse karna taaki ValueError na aaye
        msg_id = int(str(link).rstrip("/").split("/")[-1].split("?")[0])
    except ValueError:
        print(f"Download Error: Invalid message link/ID format: {link}")
        return None

    # 3 Retries taaki Telegram connection drop ko handle kar sake
    for attempt in range(3):
        try:
            msg = await app_instance.get_messages(CHAT_ID, msg_id)
            if not msg or msg.empty:
                print(f"Download Error: Message {msg_id} not found in chat.")
                return None
...
async def deliver_video_asset(app_instance, chat_id, target_user, file_path, caption, progress_callback):
    ...
    try:
        if file_id:
            # ZERO DELAY DELIVERY (Forwarding via file_id is instant)
